...
- It selects two BlueTopo tiles.
- It keeps the planned GeoTIFF plus RAT transfer below 10 MiB on 2026-07-10.
- It includes GeoTIFF and RAT sidecar URLs with SHA-256 checksums.

Expected primary tiles on 2026-07-10:

| Tile ID | Resolution (m) | UTM zone | GeoTIFF size (bytes) | RAT size (bytes) |
|---|---:|---:|---:|---:|
...
- The mixed-grid example needs an AOI that demonstrates incompatible native
  source grids.
- This AOI currently selects 4 m and 8 m BlueTopo tiles.
...
Do not silently switch AOIs during rendering. If the current NOAA catalog changes
